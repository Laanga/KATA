    ...
    const filteredResults = (data.results || [])
      .filter((item: TMDBMovie | TMDBSeries) => {
        ...
        return !excludeTitles.some(
          (excluded) => lowerTitle.includes(excluded) || excluded.includes(lowerTitle),
        );
      })
...
// ============ IGDB/RAWG (Juegos) ============
async function fetchGameRecommendations(genres: string, excludeTitles: string[], daySeed: number) {
  ...
  try {
    ...
    return (data || [])
      .filter((game: IGDBGame) => {
        ...
        return !excludeTitles.some(
          (excluded) => title.includes(excluded) || excluded.includes(title),
        );
      })
      .slice(0, 12);
  } catch {
...
// ============ Google Books (Libros) ============
async function fetchBookRecommendations(genres: string, excludeTitles: string[], daySeed: number) {
  ...
  try {
    ...
    const filteredResults = allResults
      ...
      .filter(
        (book: GoogleBookVolume, index: number, self: GoogleBookVolume[]) =>
